...
router.get('/image/:name', checkAuth, (req,  res) => {
...
router.get('/getItem/:search', checkAuth, (req, res) => {
    db.raw(`select 
        itemID,
        itemCode,
        itemName,
        ...
        perUnit
        from tbl_items 
            where LOWER(itemCode) like "%${req.params.search.toLowerCase()}%" or LOWER(itemName) like "%${req.params.search.toLowerCase()}%" AND deleteStatus = '1'
    `).then(([data]) => {
...
router.get('/moveFromToInStock', checkAuth, async(req,res) => {
    try {
